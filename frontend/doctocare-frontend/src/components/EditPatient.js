...
const EditPatient = () => {
  ...
  function handleEdit(event) {
    ...
    axios
      ...
      .then(response => {
        navigate('/patientdetail'); // Use navigate function to navigate
      })
      .catch(error => {
        console.error(error);
      });
        ...
        <form onSubmit={handleEdit}>
          <div className="mb-4">
            ...
            <input
              ...
              name="pname"
              value={inputs.pname}
              onChange={(event)=>setInputs({...inputs,pname:event.target.value})}
          ...
          <div className="mb-4">
            ...
            <input
              ...
              value={inputs.pid}
              onChange={(event)=>setInputs({...inputs,pid:event.target.value})}
              className="mt-1 p-2 border rounded w-full"
            />
            </div>
            <div className="mb-4">
            ...
            <Alert className='bg-[#f9e6ca] text-[#e65100]'  variant="ghost">
        <h1><span className='text-red-600'><FontAwesomeIcon className='text-lg md:text-base mr-2' icon={faCircleExclamation} />
        CAUTION:</span>&nbsp;There must be a doctor's ID within car001, car002, car003.</h1>
      </Alert>            </label>
            ...
            <input
              ...
              value={inputs.doc_id}
              onChange={(event)=>setInputs({...inputs,doc_id:event.target.value})}
              className="mt-1 p-2 border rounded w-full"
            />
          ...
          <div className="mb-4">
            ...
            <input
              ...
              value={inputs.date}
              onChange={(event)=>setInputs({...inputs,date:event.target.value})}
              className="mt-1 p-2 border rounded w-full"
            />
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(event)=>setInputs({...inputs,contact:event.target.value})}
              className="mt-1 p-2 border rounded w-full"
            />
          </div>
          <button
